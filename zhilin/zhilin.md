...
There is a theorem in Lie theory that states: An invariant multivariate function $f$ satisfies the necessary and sufficient condition $Gf=0$, where $G$ is the Lie group generator, see e.g. &#167; 68, page 82 in [J. E. Campbell (1903)](https://archive.org/details/introductorytrea00campuoft). Abraham Cohen (1911) and Leonard Eugene Dickson (1924) are also readable (Bourbaki-free).

P.A. Zhilin extends the theorem to the functions of multiple transforming objects of different types (vectors and matrices). The invariance requirement is posed as a functional equation. It is solved with a trick which gives a single weight to all the group parameters (I would have differentiated w.r.t. all the parameters independently, only to become lost).
